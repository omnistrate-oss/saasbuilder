...
const customDNSStatusMap: Record<string, { category: Category; label: string }> = {
  PENDING: { category: "pending", label: "Pending" },
  READY: { category: "success", label: "Ready" },
};

export const getCustomDNSStatusStylesAndLabel = (status: string): ColorObject & { label?: string } => {
